    ...
    while True:
        ...
        for block in response.content:
            if block.type == "tool_use":
                ...
                if block.name == "todo":
                    used_todo = True
        # Track how many rounds the model doesn't call todo, reset rounds_since_todo if the model update todo otherwise
        rounds_since_todo = 0 if used_todo else rounds_since_todo + 1
        # Inject nag reminder if the model doesn't call todo more than 3 rounds
        if rounds_since_todo >= 3:
